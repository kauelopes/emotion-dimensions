...
export const strings = {
  ...
  gradients: {
    ...
    text: "If emotions were discrete categories, each distance table would break into tight clusters. It doesn't. Partitioning each space's map, silhouettes sit at 0.11–0.24 in the informative spaces — statistically the same as a structureless cloud with the same spread — while a synthetic two-island reference of matched size scores 0.80. The machine consensus lands at 0.17, and its best two-way split merely rediscovers the sign of valence (ARI 0.62). The one high score is itself a lesson: RoBERTa with a template reaches 0.51, but its two \"islands\" have nothing to do with emotion (ARI with valence ≈ 0) — an extraction artifact posing as category structure. Words shade into one another: anger into irritation into annoyance. Of the three theories, this is the signature of Cowen & Keltner's semantic space: a smooth fabric organized by gradients, with valence as its strongest thread.",
  ...
  answers: {
    ...
    q3: {
      ...
      verdict: "Gradients.",
      text: "Silhouettes sit at 0.11–0.24 in the informative spaces — inside the band a structureless cloud of the same spread produces, and far from the 0.8 of a genuine two-island reference — and the machine consensus reaches only 0.17, its best two-way split merely rediscovering the sign of valence (ARI 0.62). The one exception proves the rule: RoBERTa-template scores 0.51 with a split unrelated to any emotional dimension — artifact, not category. Connectivity says the same without any clustering: the consensus's neighbour graph is a single connected piece already at k = 2, and the path from grief to joy proceeds by ordinary steps (its largest hop sits at the 31st percentile of all hops — a real island would need a 99th-percentile bridge). With 102 words this cannot adjudicate Cowen & Keltner's 27 fine-grained categories — but it is exactly the smooth, gradient-organized geometry their account predicts — the reason the semantic walk two scenes back never had to jump.",
    },
  },
  about: {
    step: "18",
